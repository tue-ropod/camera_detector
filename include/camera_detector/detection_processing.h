...
void process(const auto poseKeypoints, cv::Mat &rgbd2, camera_detector::detections &detectionData, double timeStamp, bool saveDetectionData);
void processKeypoints(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr,cv::Mat &rgbd2,camera_detector::detections &detectionData, double timeStamp, bool saveDetectionData);
};
