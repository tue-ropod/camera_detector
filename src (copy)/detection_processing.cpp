...
void detection_processing::hypothesize(gates &G,int ind,double p,hypotheses hypothesesTemp,int gate) {
...
void detection_processing::createValidationGates(vector<measurement> measurements, gates &G, vector<visitedTemp> visited) {
    vector<int> toVisitM;
    vector<int> toVisitO;
...
void detection_processing::display(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr)
{
    try
    {
...
bool detection_processing::validateSize(double dist) {
...
bool detection_processing::validateHeight(double zKnee, double zAnkle) {
...
double detection_processing::distContinuous(camera_detector::detection &m, cv::Mat &rgbd2) {
...
void detection_processing::interpolatePoints(camera_detector::detection &m, double dalpha) {
...
camera_detector::detection detection_processing::xyzPoints(const auto poseKeypoints,auto person,vector<int> D,cv::Mat &rgbd2) { // measure depth of object
        ...
        if (indx>0) { // for all detected points
            ...
            if (dist>0 && dist<20) { // only use points with distance between zero and 20 meter
                ...
                if (i==0) {
                    ...
                    m.xKnee = pTemp.x+xCamera;
                    m.yKnee = pTemp.y;
                    m.zKnee = pTemp.z;
                } else if (i==1) {
                    toXYZ(pTemp, thetaH, thetaV, dist);
                    m.xAnkle = pTemp.x+xCamera;
                    m.yAnkle = pTemp.y;
                    m.zAnkle = pTemp.z;
                }
...
void detection_processing::initializeLegs(const auto poseKeypoints, cv::Mat &rgbd2, camera_detector::detections &detectionData) {
...
void detection_processing::linkObjects(camera_detector::detections detectionData, vector<measurement> &measurements, vector<visitedTemp> &visited,int N, int M, ed_gui_server::objsPosVel &objsInfo) {
...
void detection_processing::processGates(camera_detector::detections &detectionData, gates &G) {
...
void detection_processing::processKeypoints(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr,cv::Mat &rgbd2,camera_detector::detections &detectionData,ed_gui_server::objsPosVel &objsInfo)
{
    try
    {
        ...
        if (datumsPtr != nullptr && !datumsPtr->empty())
        {
            ...
            initializeLegs(poseKeypoints,rgbd2,detectionData);
